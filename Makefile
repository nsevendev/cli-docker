...
ns: 
	...
	@$(MAKE) clean c=$(c)
	@echo "🚀 buid ns -------------> START"
	docker exec -i cli-docker sh -c "$(if $(filter $(c),d),$(BUILD_MAC_AMD),if $(filter $(c),m),$(BUILD_MAC_ARM),if $(filter $(c),m),$(BUILD_LINUX)) go build -o $(FOLDER_BUILD_LOCAL)/$(APP_NAME)"
	@echo "✅ buid ns -------------> END"
